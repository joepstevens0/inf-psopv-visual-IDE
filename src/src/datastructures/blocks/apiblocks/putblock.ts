...
// PUT works (gives correct response)
export class PUTBlock extends Block {
  public constructor() {
    super("PUT", _color);
    //Top
    ...
    this._label.setText("Update data (PUT)");
  ...
  public execute(flowcontroller : FlowController): void {
    flowcontroller.setAction(this._linkinput.getBlock(), async () => { 
      ...
      //Try to make JSON object from parameters
      let jsonstring = '{';
      ...
      if (this._enclose.getFirstBlock() != null){
        ...
        if (this._linkinput.getValue() != undefined){
          ...
          await axios.put(this._linkinput.getValue(), dataobj)
          .then((response) => {
            //Handle succes
            ...
            Logger.info("PUT-request succesful, response:", response);
          })
        ...
        //If link is not set
          ...
          //Add error to errorlist
          this._error.addError(["ERROR", "De link voor PUT is niet gezet.", solvefunc, solveparam]);
          Logger.error("POST-request error: link not set");
        }

      //If parameters aren't given
      } else {
        //Handle error
        ...
        Logger.error("PUT-request error: parameters not set");
      }
